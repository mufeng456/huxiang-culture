  ...
  setup() {
    ...
    const handleSubmit = async () => {
      try {
        ...
        if (!token) {
          error.value = '请先登录后再创建资源'
          return
        }
        
        // 调用API创建文化资源
        const response = await request('/resources', 'POST', {
        ...
        console.error('创建资源错误:', err)
        error.value = err.message || '创建资源失败，请稍后重试'
      } finally {
